...
void	handle_single_quote_tokenize(int *i, int *inside_squotes, int inside_dquotes)
{
...
void	handle_double_quote_tokenize(int *i, int *inside_dquotes, int inside_squotes)
{
		...
		t_index *index)
{
	if (str[index->i] == '\'' && !state->in_dquote && !state->in_squote)
		handle_single_quote_tokenize(&index->i, &state->in_squote, state->in_dquote);
